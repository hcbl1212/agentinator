...
describe('preload bridge', () => {
  ...
  it('routes preview capture, image, and component pinning over IPC', async () => {
    ...
    await bridge.preview.getComponent('session_9')
    await bridge.preview.setComponent(
      'session_9',
      '/app',
      'src/Cart.tsx',
      'src/Providers.tsx',
    ...
    expect(mockIpcRenderer.invoke).toHaveBeenCalledWith('preview:get-component', 'session_9')
    expect(mockIpcRenderer.invoke).toHaveBeenCalledWith(
  ...
  it('routes pipeline create, continue, and remove over IPC', async () => {
